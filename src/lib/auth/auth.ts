...
export function createApprovalRequest(
  ...
  const expiresAt = exp.toISOString().replace('T', ' ').replace(/\.\d+Z$/, ''); // 10 minutes
...
export function approveDeviceRequest(db: Database.Database, approvalId: string): boolean {
  const request = db.prepare('SELECT * FROM device_approvals WHERE id = ?').get(approvalId) as { user_id: string; device_fingerprint: string } | undefined;
  ...
  const tx = db.transaction(() => {
    ...
    const device = db.prepare('SELECT id FROM devices WHERE user_id = ? AND fingerprint_hash = ?')
      .get(request.user_id, request.device_fingerprint) as { id: string } | undefined;

    if (device) {
      trustDevice(db, device.id, 30);
    }
  });
