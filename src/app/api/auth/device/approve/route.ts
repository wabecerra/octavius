...
export async function POST(request: Request) {
  try {
    ...
    const request = getApprovalRequest(db, approvalCode.toUpperCase());
    ...
    // Approve the device
    const success = approveDeviceRequest(db, request.id);
    ...
    const device = getDeviceByFingerprint(db, request.user_id, request.device_fingerprint);
    ...
    createSessionRecord(db, request.user_id, device.id, sessionToken, expiresAt);
