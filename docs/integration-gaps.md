...
4. Obsidian bidirectional sync - fully implemented (push/pull + vault knowledge graph across 3 API endpoints)
...
3. Budget Gate incomplete - getDailySpend() always returns 0 (only STUBBED feature per deep audit)
...
8. Kanban updates not real-time synced to fleet activity
9. Sub-agent results not surfaced in task UI for multi-quadrant tasks
